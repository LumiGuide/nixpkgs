...
rec {
  ...
  paramsToRenderedStrings = cfg : ps :
    filterEmptySets (
    filterAttrsRecursive (_n : v : !isNull v)
      (mapParamsRecursive (path: name: param:
        let value = attrByPath path null cfg;
        in if (!isNull value) && value != param.option.default
           then param.render name value
           else { "${name}" = null; }
      ) ps));
  ...
  mapAttrs'' = f: set:
    fold (a: b: a // b) {} (map (attr: f attr set.${attr}) (attrNames set));
