...
in {
  ...
  options.services.postage = {
    ...
    connections = mkOption {
      ...
      description = ''
        ...
        Detailed information about PostgreSQL connection strings is available
    ...
    # TODO: the man file specifies that the local-only option exists but postage
    # crashes with "Unknown config section/name:  local_only"
    #
    # localOnly = mkOption {
    #   type = types.bool;
    #   default = false;
    ...
    tls = mkOption {
      ...
      description = ''
       ...
       https://github.com/workflowproducts/postage/blob/master/INSTALL_NGINX.md
      '';
    };
  ...
  config = mkIf cfg.enable {
    systemd.services.postage = {
      description = "PostgreSQL Administration for the web";
      wants    = [ "postgresql.service" ];
      after    = [ "postgresql.service" ];
      wantedBy = [ "multi-user.target" ];
      serviceConfig = {
        ExecStartPre = "mkdir -p ${cfg.dataRoot}";
